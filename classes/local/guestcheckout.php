...
class guestcheckout {
    ...
    public static function maybe_auto_create_guest_user_for_url(\moodle_url $url): bool {
        ...
        // If the marker of a previous attempt comes back on a still anonymous
        // request, the browser rejected the session cookie (third-party cookie
        // blocking inside an iframe). Creating another guest would redirect
        // forever, so stop here; pages can detect the marker and degrade.
        if ($url->get_param('gcattempt')) {
            return false;
        }
        ...
        // is reflected in a fresh request cycle immediately. The marker makes
        // a failed cookie round-trip detectable (see above); for logged-in
        // users it is inert.
        redirect((new \moodle_url($url->out(false), ['gcattempt' => 1]))->out(false));
        return true;
    }
