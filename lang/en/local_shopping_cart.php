...
$string['pdfaenabled'] = 'Create PDF/A-2b compliant PDFs';
$string['pdfaenabled_desc'] = 'Generate receipts / invoices (templates below) and the daily sums PDF as PDF/A-2b (archivable): all fonts are embedded, Helvetica/Arial, Times and Courier in the templates are mapped to the embeddable fonts FreeSans, FreeSerif and FreeMono, CMYK images are converted to RGB. Images referenced by URL are downloaded by the server (Moodle\'s cURL security settings apply); an image that cannot be loaded is left out and the PDF is still generated. When disabled the PDFs are generated exactly as before.';
$string['pdfahint'] = 'If "Create PDF/A-2b compliant PDFs" is enabled, this template is rendered as PDF/A-2b: use the fonts FreeSans / FreeSerif / FreeMono (Helvetica, Arial → FreeSans; Times → FreeSerif; Courier → FreeMono) and RGB images (PNG or JPEG, no CMYK).';
