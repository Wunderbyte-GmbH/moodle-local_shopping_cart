## Version 2.1.5 (2026090300)
* Improvement: The checkout page does not build the purchase history any more. It only decides whether the history tab has to be offered, and the browser fetches its content through the reload_history web service when the tab is opened. Whether a user has bought anything at all is kept in the cart store and written by the purchase itself, so the page does not query the database for it. Cancellation, rebooking and consumption keep being evaluated per request, because they depend on the current time and cart. (Wunderbyte-GmbH/moodle-local_shopping_cart#204)
* Improvement: The purchase history list does not query the database per entry any more. Installment receipts and cancellation confirmations are fetched for the whole list in two queries, allowed_to_cancel() is handed the record the caller already holds instead of reading the same row again, and the rebooking count is determined once per user. Building the list for 50 entries went from 109 to 10 database reads and no longer grows with the number of entries (about 35 ms per entry before, measured 396 ms at 7 entries against 4.016 ms at 106). (Wunderbyte-GmbH/moodle-local_shopping_cart#204)
* Bugfix: The checkout, address and guest login tabs carry the Bootstrap 4 toggle attributes again, so they keep working on Moodle 4.5.
* Bugfix: The cart item buttons (delete, increase, decrease) are initialised in the embedded checkout as well, where deleting an item did nothing.
* Improvement: The upgrade step that creates the coupon and guest checkout tables runs again under a new savepoint. Sites coming from the USI release line had a plugin version above the original savepoint and would otherwise never get those tables. All statements are idempotent, so other sites are unaffected. (Wunderbyte-GmbH/moodle-local_shopping_cart#204)
...
## Version 2.1.3 (2026081803)
* New feature: Guest checkout (GH-187) — anonymous visitors can complete a purchase without a manual account. The checkout manager was refactored to dynamic forms; guest checkout settings are unified with a configurable guest role mapping, auto-login via after_config with atomic user creation and orphan cleanup, and a new local_shopping_cart_guestusers table (also created on already-upgraded sites). A deleted or unselected address is handled gracefully instead of crashing, and the checkout navigation and payment buttons are aligned on one row.
* New feature: Coupons (GH-176) — a coupon manager applies discounts to the whole cart with a configurable price modifier, showing both the original and the reduced price in cart, checkout, history and ledger; coupons support an opt-out coupon type and are a PRO feature. A new coupons table stores the definitions.
* New feature: Item-specific booking fee — a per-item booking fee can be enabled (allowcustombookingfee).
* New feature: Plugin-agnostic embed views (local_shopping_cart\output\embed_views) let a seller plugin surface cart UI in iframes through its own embed endpoint: a compact cart badge (icon, count, total) and the full checkout (stepper, VAT, credits, coupons, addresses) rendered in an embedded layout via the shared checkout_page_data. A same-origin cart_channel keeps the badge live when the cart changes in another context, and the badge updates its price live via the get_price web service. Checkout/success redirect overrides are validated against a new embedredirectallowlist prefix setting to prevent open redirects; the payment provider redirect stays top-level.
* New feature: Add non-European option to the checkout VAT step (no VAT check) and treat GB as export destination in the VAT/tax logic (#198, #199).
* Improvement: New shopping_cart::add_partial_refund() and a new event listener that cancels payments based on subscription event listeners; context helper hardening.
* Improvement: Guest checkout auto-create no longer loops when the browser blocks the session cookie (third-party cookie blocking inside iframes); the checkout button can break out of an embedding iframe (target=_top) via a new optional flag on the navbar render callback.
* Improvement: Expose the cart badge and the checkout as shortcodes (cartbadge, cartcheckout) delegating to the embed views, so local_embed can discover and render them in iframes; both are gated by a new rotatable shortcodetoken setting (GH-195).
* Bugfix: Guest checkout auto-create must never capture the login, auth and admin areas (#197, #203).
* Bugfix: Guest checkout auto-login no longer runs on cookie-less scripts such as theme/styles.php, where it created junk guest users and 500ed the request - the whole site lost its styling after any cache purge (GH-193).
* Bugfix: The embedded checkout view no longer raises a coding_exception for a CSS file added after the header; the checkout styles are now included inline.
* Bugfix: Adding/removing cart items no longer throws in embedded contexts where the nav cart badge is absent (which also blocked the price refresh).
* Bugfix: Add missing German strings and fix string order.
