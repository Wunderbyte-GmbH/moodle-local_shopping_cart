## Version 2.1.5 (2026091801)
* New feature: Guest checkout — anonymous visitors can complete a purchase without creating an account themselves. A guest user is created on configured pages and cleaned up again when the cart was never paid, and the checkout offers a login panel so an existing account can take the cart over. The auto-create never captures the login, auth and admin areas, the catch-all pattern is limited to the front page, the dashboard and activity pages, and it does not run on cookie-less scripts such as theme/styles.php. (Wunderbyte-GmbH/moodle-local_shopping_cart#187, #197, #193, #203)
* New feature: The checkout page is built from dynamic forms — addresses, VAT number and terms and conditions are forms of their own instead of hand-built templates, and the page data is assembled in one place, so the embedded checkout shows the same content as the page. The tabs carry the Bootstrap 4 toggle attributes, so they keep working on Moodle 4.5. (Wunderbyte-GmbH/moodle-local_shopping_cart#187)
* New feature: Coupons — a coupon manager creates codes with a percentage or an absolute discount, a validity period and per-item opt-in or opt-out. A coupon limits how often it may be used in total and per user, and defines whether a use counts per checkout or per discounted item. Uses are counted in the ledger, where a coupon is recorded only on the items whose price it reduced, and a cancelled purchase gives its usage back. The booking fee is never discounted, an absolute coupon never gives away more than its value (with rounded discounts the per item share is floored, what does not fit the precision expires), and only an explicit apply changes the coupon in the browser. (Wunderbyte-GmbH/moodle-local_shopping_cart#176)
* New feature: Item-specific booking fee — with allowcustombookingfee an item can carry its own fee; the highest fee of the items in the cart wins. (Wunderbyte-GmbH/moodle-local_shopping_cart#176)
* New feature: Partial refunds — a component can refund part of a paid item as credit without cancelling the purchase, with its own ledger entry and receipt. Payments based on subscriptions can be cancelled through an event. (Wunderbyte-GmbH/moodle-local_shopping_cart#176)
* New feature: Embeddable cart views — the cart badge and the checkout are available as embed views and as the shortcodes cartbadge and cartcheckout, kept in sync across open views. (Wunderbyte-GmbH/moodle-local_shopping_cart#195)
* Improvement: The VAT step offers a non-European option without a VAT number check, and Great Britain is treated as an export destination in the tax logic. (Wunderbyte-GmbH/moodle-local_shopping_cart#198, #199)
* Improvement: The purchase history does not slow the checkout page down any more: it is loaded when the user opens the tab and fetched in two queries for the whole list. The cart history is written under the identifier of the cart, not under the one left on its items by an earlier checkout. (Wunderbyte-GmbH/moodle-local_shopping_cart#204, #207)
* Improvement: Smaller fixes and clean-up around the cash report, the installments table, the privacy provider, the caches and the PRO licence check, plus a test that guards against exit() and die() in library code.
* Documentation: The limited multi-currency support, including the currency of absolute coupons, is documented in docs/user/currencies.md. (Wunderbyte-GmbH/moodle-local_shopping_cart#208)

## Version 2.1.3 (2026081805)
* Bugfix: The shopping cart history is stored under the identifier of the cart, not under the one left on its items by an earlier checkout. write_to_db() recurses per item and took the identifier from the item, so a checkout which had been given a fresh identifier wrote its history where the previous one already had rows - the duplicate check then skipped the insert and the cart being paid ended up with no history at all. The payment gateway and deliver_order() look that cart up by its own identifier, found nothing, and a captured payment delivered nothing while the buyer was shown a success message. Reproduced in the interface: checkout in one tab without opening the payment modal, change the cart in a second tab and open the modal there, then open the modal in the first tab. (Wunderbyte-GmbH/moodle-local_shopping_cart#207)
* Improvement: Only payments started within the last 24 hours are completed automatically on checkout, down from 48 hours. (Wunderbyte-GmbH/moodle-local_shopping_cart#209)
* New feature: A site setting switches off the automatic completion of ongoing payments when the checkout page is opened. It is on by default. With it, gateways that support it are asked whether a pending payment of the user has been completed meanwhile; switching it off spares the payment provider those requests, and pending payments are then completed through the return url and the purchase notification only. (Wunderbyte-GmbH/moodle-local_shopping_cart#209)
...
* Bugfix: Fix cart init on cashier and checkout page, so the item buttons (increase/decrease/delete) keep working when the navbar cart is not displayed.
