...
function xmldb_local_shopping_cart_upgrade($oldversion) {
    ...
    if ($oldversion < 2024082800) {
        ...
        // Shopping_cart savepoint reached.
        upgrade_plugin_savepoint(true, 2024082800, 'local', 'shopping_cart');
    }

    if ($oldversion < 2024120201) {
        // As there was a problem with commit history, we repeat the adding of these fields.
        // Define field address_billing to be added to local_shopping_cart_history.
        $tablehistory = new xmldb_table('local_shopping_cart_history');
        $tableledger = new xmldb_table('local_shopping_cart_ledger');
    ...
    if ($oldversion < 2026071501) {
        ...
        $index = new xmldb_index('userid-identifier', XMLDB_INDEX_NOTUNIQUE, ['userid', 'identifier']);
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
    ...
    if ($oldversion < 2026073001) {
        local_shopping_cart_create_coupon_and_guest_tables();
    ...
    if ($oldversion < 2026090300) {
        /* The step above is skipped on sites that come from the USI release line: their plugin
        version (2026081802) was already higher than its savepoint when coupons and guest checkout
        were introduced, so they would never get the tables. The statements are idempotent, so
        running them once more costs nothing on every other site. */
        local_shopping_cart_create_coupon_and_guest_tables();
